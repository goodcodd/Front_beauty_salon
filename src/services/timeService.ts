...
export const handleTimeSelection = async (bot: TelegramBot, chatId: number, messageId: number, data: string) => {
  try {
    ...
    await bot.editMessageText(i18n.t('recordDetails', { service: service.name, master: master.name, date, time }), {
      chat_id: chatId,
...
export const handleBackToTime = async (bot: TelegramBot, chatId: number, messageId: number) => {
  try {
    const userState = getUserState(chatId);
    ...
    await bot.editMessageText(i18n.t('masterSelected', { master: userState.master.name }), {
      chat_id: chatId,
      message_id: messageId,
