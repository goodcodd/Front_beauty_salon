...
describe('handleTimeSelection', () => {
  ...
  it('should send an error message if booking details are incomplete', async () => {
    ...
    expect(sendMessageMock).toHaveBeenCalledWith(123, i18n.t('incompleteBookingDetails'));
  });
  ...
  it('should send an error message if there is an exception', async () => {
    ...
    (updateUserState as jest.Mock).mockImplementation(() => { throw new Error('Update Error'); });
...
describe('handleBackToTime', () => {
  ...
  it('should edit message with time menu if user state is valid', async () => {
    ...
    (generateTimeMenu as jest.Mock).mockReturnValue({ reply_markup: { inline_keyboard: [] } });
    
    await handleBackToTime(bot, 123, 456);
    
    expect(editMessageTextMock).toHaveBeenCalledWith(i18n.t('masterSelected', { master: 'Master 1' }), {
      chat_id: 123,
      message_id: 456,
