...
import { handleDateSelection, handleBackToDate } from '../../services/dateService';
...
describe('handleDateSelection', () => {
  ...
  it('should update user state and edit message with master menu', async () => {
    ...
    (generateMasterMenu as jest.Mock).mockResolvedValue({ reply_markup: { inline_keyboard: [] } });
    
    await handleDateSelection(bot, 123, 456, 'date_2024-10-12');
    
    expect(updateUserState).toHaveBeenCalledWith(123, { date: '2024-10-12' });
    expect(editMessageTextMock).toHaveBeenCalledWith(i18n.t('dateSelected', { date: '2024-10-12' }), {
      chat_id: 123,
      message_id: 456,
  ...
  it('should send an error message if there is an exception', async () => {
    ...
    (generateMasterMenu as jest.Mock).mockRejectedValue(new Error('Menu Error'));
...
describe('handleBackToDate', () => {
  ...
  it('should edit message with date menu', async () => {
    ...
    (generateDateMenu as jest.Mock).mockReturnValue({ reply_markup: { inline_keyboard: [] } });
    
    await handleBackToDate(bot, 123, 456, 1);
    
    expect(editMessageTextMock).toHaveBeenCalledWith(i18n.t('serviceSelected', { service: 'Service 1' }), {
      chat_id: 123,
      message_id: 456,
      reply_markup: { inline_keyboard: [] },
    });
