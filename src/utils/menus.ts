...
export const generateServiceMenu = async (locale: string = 'en') => {
  try {
    ...
    return {
      reply_markup: {
        inline_keyboard: data.map((service: { name: string, documentId: string }) => [
          { text: service.name, callback_data: `service_${service.documentId}` },
        ]),
      },
    };
  } catch (error) {
...
export const generateMasterMenu = async (serviceId: number, isBack: boolean = true, locale: string = 'en') => {
  try {
    ...
    const inlineKeyboard = data
      .map((master: { documentId: string, name: string }) => [{ text: master.name, callback_data: `master_${master.documentId}` }]);
    
    if (isBack) {
      inlineKeyboard.push([{ text: i18n.t('back'), callback_data: 'back_to_date' }]);
    }
...
export const generateDateMenu = (weekOffset: number = 0) => {
  ...
  const chunkedDays = chunkArray(days.map(day => ({ text: day, callback_data: `date_${day}` })), 3);
  ...
  return ({
    reply_markup: {
      inline_keyboard: [
        ...chunkedDays,
        [
          { text: i18n.t('prevWeek'), callback_data: `prev_week_${weekOffset - 1}` },
          { text: i18n.t('nextWeek'), callback_data: `next_week_${weekOffset + 1}` }
        ],
